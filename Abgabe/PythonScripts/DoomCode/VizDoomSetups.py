...
def state_machine_environment():
    ...
    game.load_config(os.path.join(_script_dir, "../../Data/environments/deathmatch.cfg"))
    game.set_doom_game_path(os.path.join(_script_dir, "../../Data/environments/DOOM2.wad"))
    ...
    while True:
        if game.is_episode_finished():
            print("Got", game.get_game_variable(vzd.GameVariable.KILLCOUNT), "kills")
            machine = StateMachine(game_width=game.get_screen_width())
            game.new_episode()
            frame_count = 0
...
def excercise_environment(num_episodes=30, machine=None):
    ...
    game.close()
    print("Mean kills over", num_episodes, "episodes:", sum(total_kills)/num_episodes)
...
def state_machine_environment_debug():
    ...
    game.load_config(
        os.path.join(
            _script_dir,
            "../../Data/environments/deathmatch.cfg"
        )
    )
