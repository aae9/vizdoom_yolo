...
class StateMachine():
    def __init__(self, game_width):
        ...
        self.weapon_switch_cooldown = 100
        #Health 
    ...
    def main(self, results, values, frame_count):
        self.action = [0] * 20
        self.weapon_switch_cooldown = max(0, self.weapon_switch_cooldown - 1) # Decrease weapon switch cooldown
        if frame_count % 5 == 0 or self.target is None:
            ...
            if self.weapon_switch_cooldown == 0: # Only switch weapons if not in cooldown
    ...
    def switch_weapons(self, selected_weapon, current_ammo):
        ...
        #Switch to Shotgun if enemy is close
        ...
        if current_ammo == 0 or selected_weapon != self.current_weapon or selected_weapon == 0:
            ...
            if self.weapon_ammo_memory[6] > 0:
                ...
                self.action[11] = 1 # Select weapon 3
        ...
        self.weapon_switch_cooldown = 20 # Set cooldown after switching
    ...
    def _go_kill(self):
        ...
        turn = distance * gain
        turn = max(-max_turn, min(max_turn, turn))
        sprite_size = (self.target["boundarybox"][2] - self.target["boundarybox"][0]) * (self.target["boundarybox"][3] - self.target["boundarybox"][1])
        if turn > 0:
            self.action[turn_left_right_delta] = turn
            self.action[attack] = should_shoot
            self.action[move_right] = 1
            if sprite_size < 1000: # If the enemy is far, move forward, if close, move backward
                self.action[move_forward] = 1
            else:    
